...
var _ = require('lodash'),
  ...
  buildGlob = (function() {
    /*jslint stupid:true */
    return fs.readFileSync('.npmignore', {
      encoding: 'utf8'
  ...
  configureGrunt = function(grunt) {
    ...
    var cfg = {
      ...
      sass: {
        compress: {
          ...
          files: [{
            dest: path.resolve('core/client/assets/css/<%= pkg.name %>.min.css'),
            src: path.resolve('core/client/assets/sass/screen.scss')
          }, {
            dest: path.resolve('core/client/docs/dist/css/<%= pkg.name %>.min.css'),
            src: path.resolve('core/client/assets/sass/screen.scss')
          }]
        }
      },
    ...
    grunt.registerTask('validate', 'Run tests and lint code', ['init', 'test']);
    ...
    //
    grunt.registerTask('test', 'Run tests and lint code', ['jshint', 'jscs', 'test-routes', 'test-module', 'test-unit', 'test-integration', 'test-functional']);
    ...
    grunt.registerTask('test-unit', 'Run unit tests (mocha)', ['clean:test', 'setTestEnv', 'ensureConfig', 'mochacli:unit']);
    ...
    //
    // A coverage report can be generated for these tests using the `grunt test-coverage` task.
    grunt.registerTask('test-integration', 'Run integration tests (mocha + db access)', ['clean:test', 'setTestEnv', 'ensureConfig', 'mochacli:integration']);
    ...
    grunt.registerTask('test-routes', 'Run functional route tests (mocha)', ['clean:test', 'setTestEnv', 'ensureConfig', 'mochacli:routes']);
    ...
    grunt.registerTask('test-module', 'Run functional module tests (mocha)', ['clean:test', 'setTestEnv', 'ensureConfig', 'mochacli:module']);
    ...
    grunt.registerTask('test-functional-setup', 'Run functional tests for setup', ['clean:test', 'setTestEnv', 'ensureConfig', 'cleanDatabase', 'express:test',
    ...
    grunt.registerTask('test-functional', 'Run functional interface tests (CasperJS)', ['clean:test', 'setTestEnv', 'ensureConfig', 'cleanDatabase', 'express:test', 'spawnCasperJS', 'express:test:stop',
    ...
    grunt.registerTask('test-coverage', 'Generate unit and integration (mocha) tests coverage report', ['clean:test', 'setTestEnv', 'ensureConfig', 'shell:coverage']);
    ...
    grunt.registerTask('emberBuildDev', 'Build Ember JS & templates for development', ['clean:tmp', 'buildAboutPage', 'emberTemplates:dev', 'transpile', 'concat_sourcemap:dev']);

    // ### Ember Build *(Utility Task)*
    // All tasks related to building the Ember client code including transpiling ES6 modules and building templates
    grunt.registerTask('emberBuildProd', 'Build Ember JS & templates for production', ['clean:tmp', 'buildAboutPage', 'emberTemplates:prod', 'transpile', 'concat_sourcemap:prod']);

    // ### CSS Build *(Utility Task)*
    // Build the CSS files from the SCSS files
    grunt.registerTask('css', 'Build Client CSS', ['sass', 'autoprefixer']);
    ...
    grunt.registerTask('init', 'Prepare the project for development', ['shell:bower', 'update_submodules', 'default']);

    // ### Production assets
    // `grunt prod` - will build the minified assets used in production.
    //
    // It is otherwise the same as running `grunt`, but is only used when running Ghost in the `production` env.
    grunt.registerTask('prod', 'Build JS & templates for production', ['concat:prod', 'copy:prod', 'emberBuildProd', 'uglify:prod']);
    ...
    grunt.registerTask('default', 'Build JS & templates for development', ['concat:dev', 'copy:dev', 'css', 'emberBuildDev']);
    ...
    grunt.registerTask('dev', 'Dev Mode; watch files and restart server on changes', ['default', 'express:dev', 'watch']);
    ...
    grunt.registerTask('release',
      ...
      ' - Zip files in release-folder to dist-folder/#{version} directory', ['init', 'concat:prod', 'copy:prod', 'emberBuildProd', 'uglify:release', 'clean:release', 'copy:release', 'compress:release']);
  };
